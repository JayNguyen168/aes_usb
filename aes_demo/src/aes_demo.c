...
static void KeyExpansion(uint8_t *roundKey, const uint8_t *key) {
    ...
    for (i=4;i<44;i++) {
        ...
        for(j=0;j<4;j++) roundKey[i*4+j]=roundKey[(i-4)*4+j]^temp[j];
    }
}
...
void AES_encrypt(AES_ctx *ctx,uint8_t *buf){
    uint8_t state[16]; memcpy(state,buf,16);
    ...
    AddRoundKey(state,ctx->round_key+AES_ROUNDS*16);
    memcpy(buf,state,16);
}
